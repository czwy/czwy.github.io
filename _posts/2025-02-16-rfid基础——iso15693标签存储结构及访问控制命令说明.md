...
ISO 15693标签存储区说明#^f04f63
ISO 15693标签存储区说明#Block -4 & Block -3
ISO 15693标签存储区说明#Block -2
ISO 15693标签存储区说明#Block -1
## 传输协议
...
上述示例中，响应的信息标志为 `0x0F`，表示 DSFID 、 AFI、VICC内存容量、IC参考都会在结果中显示。其中信息域中VICC内存容量的值为`1B 03`，由于<a href="#传输协议">RFID基础——ISO15693标签存储结构及访问控制命令说明#传输协议|传输协议</a>中定义了RFID基础——ISO15693标签存储结构及访问控制命令说明#^abadd7</a>，因此 `1B` 是表示有27个数据块，`03` 表示每块4个字节。
